...
bool Texture::loadFromFile(std::string path)
{
	...
	else
	{
		...
		newTexture = SDL_CreateTextureFromSurface(PlayLayer::renderer, loadedSurface);
		if (newTexture == NULL)
		{
...
/*bool Texture::loadFromRenderedText(std::string textureText, SDL_Color textColor, TTF_Font *textFont)
{
	...
	{
		// Create texture from surface pixels
		texture = SDL_CreateTextureFromSurface(PlayLayer::renderer, textSurface);
		if (texture == NULL)
		{
...
void Texture::render(int x, int y, int width, int height, SDL_Rect *clip, double angle, SDL_Point *center, SDL_RendererFlip flip)
{
	...
	// Render to screen
	if(SDL_RenderCopyEx(PlayLayer::renderer, texture, clip, &renderQuad, angle, center, flip)!= 0)
	{
